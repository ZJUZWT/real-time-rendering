...
//GLuint sphereVBO = 0;
//GLuint sphereEBO = 0;
//void renderSphere(glm::mat4 M, glm::mat4 V, glm::mat4 P,GLuint shader) {
//	const unsigned int X_SEGMENTS = 64;
//	const unsigned int Y_SEGMENTS = 64;
//	if (!sphereVAO) {
//		const float PI = 3.14159265359;
//		float* sphereVertices = (float*)malloc(8 * (X_SEGMENTS + 1) * (Y_SEGMENTS + 1) * sizeof(float));
...
//
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 0] = xPos;
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 1] = yPos;
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 2] = zPos;
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 3] = xSegment;
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 4] = ySegment;
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 5] = xPos;
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 6] = yPos;
//				sphereVertices[(y * (X_SEGMENTS + 1) + x) * 8 + 7] = zPos;
//			}
//		}
//		unsigned int* sphereElements = (unsigned int*)malloc(2 * (X_SEGMENTS + 1) * Y_SEGMENTS * sizeof(unsigned int));
//		bool oddRow = false;
//		for (unsigned int y = 0; y < Y_SEGMENTS; ++y) {
//			if (!oddRow) // even rows: y == 0, y == 2; and so on
//				for (unsigned int x = 0; x <= X_SEGMENTS; ++x) {
//					sphereElements[(y * X_SEGMENTS + x) * 2 + 0] = y * (X_SEGMENTS + 1) + x;
//					sphereElements[(y * X_SEGMENTS + x) * 2 + 1] = (y + 1) * (X_SEGMENTS + 1) + x;
//				}
//			else
//				for (int x = X_SEGMENTS; x >= 0; --x) {
//					sphereElements[(y * X_SEGMENTS + X_SEGMENTS-x) * 2 + 0] = (y + 1) * (X_SEGMENTS + 1) + x;
//					sphereElements[(y * X_SEGMENTS + X_SEGMENTS-x) * 2 + 1] = y * (X_SEGMENTS + 1) + x;
//				}
//			oddRow = !oddRow;
//		}
//		//创建句柄
...
//		//填充内存
//		glNamedBufferStorage(sphereVBO, sizeof(sphereVertices), sphereVertices, 0);
//		glNamedBufferStorage(sphereEBO, sizeof(sphereElements), sphereElements, 0);
//		//绑定
...
//		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), 0);
//		glEnableVertexAttribArray(0);
//		glVertexAttribPointer(1, 2, GL_FLOAT, GL_TRUE, 8 * sizeof(GLfloat), (void*)(3 * sizeof(GLfloat)));
//		glEnableVertexAttribArray(1);
...
//
//	glBindVertexArray(sphereVAO);
//	glDrawArrays(sphereVAO, 0, 8 * (X_SEGMENTS + 1) * (Y_SEGMENTS + 1));
//	//glDrawElements(GL_TRIANGLE_STRIP, 2 * (X_SEGMENTS + 1) * Y_SEGMENTS, GL_UNSIGNED_INT, 0);
//}
